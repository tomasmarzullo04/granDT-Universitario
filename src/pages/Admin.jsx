...
export default function Admin() {
  const [activeTab, setActiveTab] = useState('armado'); // 'armado' | 'stats'

  return (
    <AdminLayout>
      <div className="w-full h-full flex flex-col">
        {/* Sub-nav Local (Tabs) */}
        <div className="flex bg-white p-1 rounded-2xl border border-neutral/20 shadow-sm max-w-md mb-6">
          <button
        ...
        <div className="bg-white rounded-3xl border border-neutral/20 shadow-xl p-4 md:p-8 animate-fade-in min-h-[600px]">
          {activeTab === 'armado' ? <AdminDragDropBuilder /> : <ResultadosAdmin />}
        </div>
      </div>
    </AdminLayout>
  );
